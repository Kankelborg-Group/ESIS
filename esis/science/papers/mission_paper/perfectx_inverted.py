...
if __name__ == '__main__':
    ...
    for j, seq in enumerate(seqs):
        ...
        intensity = axs_top[j].imshow(int_cutout, vmax=350)
    ...
    for j, seq in enumerate(seqs):
        for i in range(event_pix.shape[1]):
            ...
            flat_axs[i].annotate('v =' + vel1 + ', ' + vel2, (150, top - 3- 2 * j), color=colors[j], fontsize=7)
    ...
    plt.show()
    fig1.savefig(fig_path / 'perfect_x_inverta.pdf')
    fig2.savefig(fig_path / 'perfect_x_invertb.pdf')
    plt.show()
